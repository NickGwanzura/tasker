...
export const quotes = taskerSchema.table("quotes", {
  ...
  items: jsonb("items").$type<Array<{ description: string; details?: string; quantity: number; rate: number; amount: number }>>().notNull().default([]),
...
export const invoices = taskerSchema.table("invoices", {
  ...
  clientEmail: text("client_email").notNull(),
  clientAddress: text("client_address").notNull().default(""),
  items: jsonb("items").$type<Array<{ description: string; details?: string; quantity: number; rate: number; amount: number; discount?: number }>>().notNull().default([]),
  subtotal: integer("subtotal").notNull().default(0),
